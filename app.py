...
def buy():
    ...
    if request.method == "POST":
        ...
        if not symbol:
            ...
            return apology("Must provide positive number of shares", 402)
        ...
        if cash < total_cost:
            return apology("Not enough cash", 402)
...
def register():
    ...
    if request.method == "POST":
        ...
        rows = db.execute("SELECT * FROM users WHERE username =?", request.form.get("username"))
        if len(rows) != 0:
           return apology("Username already exists", 401)
        #Add new user
...
def sell():
    ...
    if request.method == "POST":
        ...
        if not symbol:
            ...
            return apology("Must provide a positive number of shares", 405)
        else:
            shares = int(shares)
